    ...
    while i >= 0:
        ...
        if float(price) < float(trades_price[i]) or float(price) > float(trades_price2[i]):
            ...
            with open("history.txt", "a") as f:
                f.write("Sell - " + trades[i] + "  " + '{:5.4f}s'.format(price) + "\n")
            ...
            for line in f:
                ...
                if contains == False:
                    if sar == 0:
                        ...
                        f.write("Buy - " + symbol + "  " + bricks[len(bricks) - 1][4] + " - " + '{:5.4f}s'.format(sar) + " - " + '{:5.4f}s'.format((float(bricks[len(bricks) - 1][4]) - sar) + float(bricks[len(bricks) - 1][4])) + "\n")
                        f.close()
